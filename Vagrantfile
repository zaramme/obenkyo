...
Vagrant.configure("2") do |config|
  ...
  config.vm.synced_folder "./sync", "/vagrant", mount_options: ['dmode=777','fmode=777']
  ...
  # 初期設定(必要ない場合は適宜コメントアウト)
  config.vm.provision "file", source: "provision/php.ini", destination: "php.ini"
  config.vm.provision "file", source: "provision/httpd.conf", destination: "httpd.conf"
